...
import { X, Eye, Edit, Trash2, Plus, MoreHorizontal, ChevronDown, Grid, List, ArrowUp, ArrowDown } from "lucide-react"
...
export default function ProjectsPage() {
  ...
  const filteredAndSortedProjects = useMemo(() => {
    const filtered = (projectsData || []).filter((project: any) => {
      ...
      const priorityFilters = activeFilters.filter(f => f.type === 'priority').map(f => f.value)

      const matchesStatus = statusFilters.length === 0 || statusFilters.includes(project.status)
      const matchesPriority = priorityFilters.length === 0 || priorityFilters.includes(project.priority)
    ...
    return filtered
  }, [projectsData, searchTerm, sortBy, sortDirection, activeFilters])
  ...
  const addFilter = (type: 'status' | 'priority', value: string) => {
    const filterId = `${type}-${value}`
    if (!activeFilters.some(filter => filter.id === filterId)) {
      setActiveFilters(prev => [...prev, { type, value, id: filterId }])
    }
  }
                      ...
                      <div className="space-y-2">
                        {SORT_OPTIONS.map((option) => (
                          <div key={option.value} className="flex items-center justify-between">
                            <span 
                              className={`text-sm cursor-pointer ${sortBy === option.value ? "font-semibold text-blue-600" : ""}`}
                            ...
                            {sortBy === option.value && (
                              <div className="flex gap-1">
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className={`h-6 w-6 ${sortDirection === 'asc' ? 'bg-gray-100' : ''}`}
                                ...
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className={`h-6 w-6 ${sortDirection === 'desc' ? 'bg-gray-100' : ''}`}
                      ...
                      {uniqueStatuses.map((status) => (
                        <label key={status} className="flex items-center gap-2 mb-2 cursor-pointer">
                          <input
                            ...
                            onChange={() => addFilter('status', status)}
                            className="rounded"
                          />
                    ...
                    <div className="p-3">
                      ...
                      {uniquePriorities.map((priority) => (
                        <label key={priority} className="flex items-center gap-2 mb-2 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={activeFilters.some(f => f.type === 'priority' && f.value === priority)}
                            onChange={() => addFilter('priority', priority)}
                            className="rounded"
                          />
